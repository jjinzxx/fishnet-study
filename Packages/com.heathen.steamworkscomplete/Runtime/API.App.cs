...
namespace HeathenEngineering.SteamworksIntegration.API
{
    ...
    public static class App
    {
        ...
        public static class Client
        {
            ...
            public static void Initialize(AppData appId) => Initialize(appId, null, null, null);
            /// <summary>
            /// Start the initialization process for Steam API given a specific App ID and artifact settings
            /// </summary>
            /// <param name="appId">The ID of the app to request initialization as</param>
            /// <param name="inventory">Settings for Steam Inventory, <see cref="InventorySettings"/> for more information.</param>
            /// <param name="leaderboards">A collection of <see cref="LeaderboardObject"/>s to initialize</param>
            /// <param name="actions">A collection of <see cref="InputAction"/>s to initialize</param>
            ...
            {
                Debug.Log("Initializing for App ID = " + appId.ToString());
                if (Initialized)
                {
                ...
                else
                {
                    ...
                    {
                        Debug.Log($"RestartAppIfNecessary = true, {appId} is not the AppID expected");
                        Application.Quit();
                    }
#else
                    ...
                    if (Initialized)
                    {
                        ...
                        // Valve에서 폐기한 전체 앱 목록 API를 호출하므로 자동 로드를 사용하지 않습니다.
                        // 현재 멀티플레이 로비 및 P2P 연결 검증에는 앱 이름 목록이 필요하지 않습니다.
                        ...
                        {
                            LeaderboardData.GetAll(leaderboards, result =>
                            {
                            ...
                            && actions.Length > 0)
                        {
                            ...
                            var actionCollection = new (string name, InputActionType type)[actions.Length];
                            for(int i = 0; i < actions.Length; i++)
                                actionCollection[i] = (actions[i].ActionName, actions[i].Type);
                            
                            if(!Input.Client.Init(actionCollection))
                            {
